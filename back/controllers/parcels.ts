...
export const getParcels = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (req.query.sender && typeof req.query.sender === "string" && req.query.sender.trim() !== "") {
            ...
            else return res.send({ parcels: [], hasMore: false, currentPage: page, total: 0 });
        }
        ...
        const parcels = await Parcel.find(query).populate("sender").populate("recipient").sort({ draftedAt: -1 }).skip(skip).limit(limit);
...
export const updateParcelStatus = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const validStatuses = ["draft","created","accepted","shipped","in_country","in_city","at_pickup_point","delivered"];
        if (!validStatuses.includes(status)) return res.status(400).json({ error: "Invalid status", validStatuses, providedStatus: status });
        ...
        res.json({ message: "Parcel status updated successfully", parcel: freshParcel });
    } catch (e) {
        next(e);
    }
};
