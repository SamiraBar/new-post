...
export const adminDelete = async (
  ...
  try {
    ...
    if (removeUser.role === "superAdmin")
      return res.status(400).send({ error: "Cannot delete super admin" });
...
export const adminEditSelf = async (
  ...
  try {
    ...
    const currentAdmin = adminReq.admin;

    if (currentAdmin.role !== "superAdmin") {
      return res
        .status(403)
        .send({ error: "Only super admin can edit their profile" });
    }
