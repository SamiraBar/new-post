            ...
            >
              {Array.from({ length: totalGridCols }).map((_, idx) => {
                if (idx % 2 === 0) {
                  ...
                  return (
                    <div key={`circle-${step.id}`} className="flex justify-center">
                      <div
              ...
              >
                <div
                  className={`mb-3 transition-all duration-300 relative ${
                    step.id <= currentStep ? `${step.color} scale-110` : 'text-gray-400 scale-100'
                  }`}
                >
                  {step.icon}
                  {pulsingStep === step.id && (
                ...
                >
                  <div className="font-semibold mb-1 leading-tight">{step.label}</div>
