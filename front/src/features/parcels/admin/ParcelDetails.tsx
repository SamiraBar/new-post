...
const ParcelDetails = () => {
  ...
  const { parcel, getParcelById, getParcelLoading, getParcelError } = useParcelsStore();
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white space-y-10 text-[15px] text-gray-900">
          ...
          <div className="md:col-span-8 border-b md:border-b-0 md:border-r border-gray-300">
            <div className="divide-y divide-gray-200">
              {[
                ...
                ["Дата создания", new Date(parcel.createdAt).toLocaleString()],
                ["Дата оплаты", parcel.isPaid ? "Оплачено" : "Не оплачено"],
                ["Трек номер партнера", parcel.partnerTrackingNumber || "-"],
              ].map(([label, value], i) => (
                <div key={i} className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 px-3 sm:px-4 py-2.5">
                  <span className="text-sm text-gray-700 whitespace-nowrap">{label}</span>
                  <div className="flex items-center gap-1.5">
                    <span className="bg-gray-100 border border-gray-300 px-2 sm:px-3 py-1 rounded text-gray-800 text-xs sm:text-sm break-all max-w-[180px] sm:max-w-none text-right">{value}</span>
      ...
      <Card className="border-gray-300 rounded-md overflow-hidden">
        <div className="grid grid-cols-1 md:grid-cols-2 divide-x divide-gray-300">
          <div className="p-6">
            <h3 className="font-extrabold text-lg uppercase mb-5 text-center tracking-wide">ОТПРАВИТЕЛЬ</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-3"><User className="text-gray-600" size={18} /><span>{parcel.sender.fullName}</span></div>
              <div className="flex items-center gap-3"><Phone className="text-gray-600" size={18} /><span>{parcel.sender.phoneNumber}</span></div>
              <div className="flex items-center gap-3"><Mail className="text-gray-600" size={18} /><span>{parcel.sender.email}</span></div>
              ...
              <div className="flex justify-between border-t border-gray-300 pt-3 mt-4">
                ...
                <div>
                  ...
                  <p className="font-bold text-gray-900 text-lg">{parcel.isPaid ? "1205.60" : "0.00"}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            <h3 className="font-extrabold text-lg uppercase mb-5 text-center tracking-wide">ПОЛУЧАТЕЛЬ</h3>
            <div className="space-y-3">
              <div className="flex items-center gap-3"><User className="text-gray-600" size={18} /><span>{parcel.recipient.fullName}</span></div>
              <div className="flex items-center gap-3"><Phone className="text-gray-600" size={18} /><span>{parcel.recipient.phoneNumber}</span></div>
              <div className="flex items-center gap-3"><Mail className="text-gray-600" size={18} /><span>{parcel.recipient.email}</span></div>
              <div className="flex items-center gap-3">
                <MapPin className="text-gray-600" size={18} />
                <span>{parcel.sender.address || "Адрес не указан"}</span>
              </div>
