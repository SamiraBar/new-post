...
const Step5Review: FC<Props> = ({ order, doorDelivery }) => {
  ...
  const hasPvz = !!order.destinationPvzCode;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">
                  {t("deliveryCostCalculator.stepFive.where")}
                </p>
              ...
              <div>
                ...
                {doorDelivery ? (
                    ...
                    <>
                      <p className="text-sm text-gray-600">
                        ПВЗ: {order.destinationPvzName || order.destinationPvzCode}
                      </p>
                      {order.destinationPvzAddress && (
                          <p className="text-xs text-gray-500 mt-1">
                            {order.destinationPvzAddress}
                          </p>
                      )}
                      {(order.destinationPvzPhone || order.destinationPvzWorktime) && (
                          <p className="text-xs text-gray-500 mt-1">
                            {order.destinationPvzPhone && `Тел.: ${order.destinationPvzPhone}`}
                            {order.destinationPvzPhone && order.destinationPvzWorktime && " • "}
                            {order.destinationPvzWorktime &&
                                `Часы работы: ${order.destinationPvzWorktime}`}
                          </p>
                      )}
                    </>
                ) : (
                    <p className="text-sm text-gray-600">
                      Офис #{order.destinationOffice || "—"}
                    </p>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-500">
                  {t("deliveryCostCalculator.stepFive.price")}
                </p>
                <p className="font-semibold">{order.parcelValue} сом</p>
              </div>
              <div>
                <p className="text-sm text-gray-500">
                  {t("deliveryCostCalculator.stepFive.weight")}
                </p>
              ...
              <div>
                ...
                <p className="font-semibold">
                  {order.inParcel || "Не указано"}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-500">
                  {t("deliveryCostCalculator.stepFive.date")}
                </p>
                <p className="font-semibold">
                  {order.deliveryDate || "Не указано"}
                </p>
              </div>
            </div>
          </div>
