...
const AdminCalculatorSettings = ({
  ...
  const loadCalculatorData = async (lng: Lang) => {
    setLoading(true);
    try {
      const { data: limitsResponse } = await axiosApi.get<CalcLimits>('/calculator-limits');
        ...
        <CardContent className="space-y-4 pb-8">
          ...
          <div className="mt-4 rounded-lg border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">
            <div className="flex items-start gap-2">
              ...
              <p>
                Эти значения используются для валидации полей калькулятора. Убедитесь, что обновили
                тексты уведомлений ниже после изменения этих значений.
              </p>
            </div>
          </div>
        </CardContent>
        ...
        <CardContent className="space-y-6 pb-8">
          <div>
            ...
            <Textarea
              ...
              onChange={(e) => onNoticesChange({ ...calcNotices, warningDelivery: e.target.value })}
              rows={3}
              className="mt-2"
